...
const ANNOTATIONS_PAYLOAD = {
    ...
    annotations: [
        ...
        },
        {
            ...
            label: "MaterialTheme.typography.bodyMedium · 14.0sp/20.0sp · Roboto Flex · 400 · tracking 0.25sp",
...
const FIXTURE_STATES = [
    ...
    },
    {
        ...
        apply: async (page) => {
            ...
            await page.waitForFunction(
                () =>
                    document.querySelectorAll("svg .cp-page-replaced").length >
                    0,
            );
            await page.waitForFunction(
                () =>
        ...
        apply: async (page) => {
            ...
            const semantics = await page.evaluate(() => {
                const spots = Array.from(
                    document.querySelectorAll(".cp-page-node"),
                );
                return {
                    anchors: spots.filter(
                        (s) => s.tagName.toLowerCase() === "a",
                    ).length,
                    pressed: spots.filter((s) => s.hasAttribute("aria-pressed"))
                        .length,
                };
            });
            ...
            const selfMatch = await page.evaluate(async () => {
                ...
                const result = await window.ComposePreviewCompare.scoreImages(
                    img,
                    img,
                );
        ...
        apply: async (page) => {
            ...
            await page.waitForFunction(() => {
                ...
                return (
                    ...
                    !bar.hidden &&
                    level &&
                    parseInt(level.textContent, 10) > 150
                );
            });
    ...
    },
    {
        ...
        apply: async (page) => {
            ...
            const marks = await page.evaluate(() => {
                ...
                return {
                    zoom: parseFloat(
                        getComputedStyle(stage).getPropertyValue(
                            "--cp-page-zoom",
                        ),
                    ),
    ...
    },
    {
        ...
        apply: async (page) => {
            ...
            await page.waitForSelector(
                ".cp-history-menu[open] .cp-history-list",
            );
        },
    },
    {
    ...
    },
    {
        ...
        apply: async (page) => {
            ...
            // Typography follows the compact, one-line style summary introduced by #3677. Pin both
            // halves: it must stay on one line, and widening the content may not silently clip it.
            await expect(
                page.locator(
            ...
            await page.route("**/usage/**", (route) =>
                route.fulfill({
                    ...
                    body: JSON.stringify({
                        ...
                        residue: [],
                        blobUrl:
                            "https://github.com/example/catalog/blob/main/Buttons.kt",
                        playgroundHref:
    ...
    },
    {
        ...
        apply: async (page) => {
            ...
            await page.click(
                `[data-theme-choice="${wantDark ? "dark" : "light"}"]`,
            );
            await page.waitForFunction(
    ...
    })),
    {
        ...
        apply: async (page) => {
            ...
            await page.waitForFunction(
                () =>
                    document.getElementById("cp-live-toggle-label")
                        ?.textContent === "Java",
            );
        },
    },
    ...
    },
    {
        ...
        apply: async (page) => {
            ...
            await page.waitForSelector(
                "#cp-site-menu[open] + .cp-site-menu-panel",
            );
    ...
    },
    {
        ...
        apply: async (page) => {
            ...
            await page.waitForFunction(
                () =>
                    document.querySelector(".cp-live-chip")?.textContent ===
                    "live",
            );
        },
    },
    ...
    },
    {
        ...
        apply: async (page) => {
            ...
            await expect(page.locator("#cp-explode-tilt-value")).toHaveText(
                "46°",
            );
    ...
    },
    {
        ...
        apply: async (page) => {
            ...
            await page.waitForFunction(() => {
                const started = document.querySelectorAll(
                    ".cp-rc-row[data-scored]",
                );
        ...
        test(`snapshot · ${fixture} · ${theme}`, async ({ page }) => {
            ...
            for (const lane of IMAGE_LANES) {
                await page.route(lane, (route) => {
                    ...
                    const exploded =
                        svg && url.searchParams.get("exploded") === "1";
            ...
            if (fixture === "serve-parity") {
                await page
                    .waitForFunction(() => {
                        ...
                        // count or "All".
                        return (
                            status && !/^Check/.test(status.textContent.trim())
                        );
                    })
                    .catch(() => {});
            }
            ...
            for (const state of FIXTURE_STATES.filter(
                (s) => s.fixture === fixture,
            )) {
...
test("contract · the sidebar filter follows the pane it is pointed at", async ({
    ...
    await page.goto(
        "/preview-harness/fixtures/pages/serve-landing-grouped.html",
    );
...
test("contract · Back to an unthemed entry hands the chrome back to the OS", async ({
    ...
    const scheme = () =>
        page.evaluate(
            () =>
                (document.documentElement.className.match(/cp-scheme-\w+/) || [
                    "",
                ])[0],
        );
...
test("contract · the render history is a menu in the toggle row, and the fit cap agrees", async ({
    ...
    await page.goto(
        "/preview-harness/fixtures/pages/serve-viewer-history.html",
    );
    ...
    const { applied, expected, inToggleRow, stageSibling, closed } =
        await page.evaluate(() => {
            ...
            const top =
                stage.getBoundingClientRect().top + (window.scrollY || 0);
            return {
                applied: parseInt(
                ...
                expected: Math.max(
                    320,
                    Math.round(window.innerHeight - top - 64),
                ),
