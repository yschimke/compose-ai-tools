// Compose Preview webview script.
//
// Loaded by `previewPanel.ts` via `<script nonce=... src=...
// data-early-features=...>`. Browser-only — no Node / VS Code APIs beyond the
// `acquireVsCodeApi()` postMessage bridge. The `earlyFeaturesEnabled` flag is
// threaded in through the script tag's `data-early-features` attribute so the
// host TS file no longer carries the entire panel as an inline template.

(function (earlyFeaturesEnabled) {
...
})(document.currentScript?.dataset?.earlyFeatures === "true");
