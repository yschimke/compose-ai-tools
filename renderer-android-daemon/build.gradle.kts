...
plugins {
  ...
  // D-harness.v2 — `PreviewManifestRouter` (mirrors :renderer-desktop-daemon's) reads a JSON
  ...
  // `:renderer-daemon-core`'s `api(libs.kotlinx.serialization.json)`.
...
android {
  ...
  // `:tools:daemon-harness`'s test runtime classpath can pull them via
  // `testImplementation(testFixtures(project(":renderer-android-daemon")))` — same shape
  // `:renderer-desktop-daemon` already uses for the desktop testFixtures.
  //
...
dependencies {
  ...
  // `:tools:daemon-harness`'s test runtime classpath via
  // `testImplementation(testFixtures(project(":renderer-android-daemon")))`. Mirrors
  // `:renderer-desktop-daemon`'s testFixtures wireup. The Compose foundation/runtime/ui surface
  // is the only piece the fixture composables touch — listed compileOnly because the consumer
  // (`renderer-android` for unit tests, `:tools:daemon-harness` for the real-mode S1-S8 Android
...
// :renderer-android-daemon:test`) — same classes the daemon needs at runtime when spawned as a
...
// D-harness.v2 — the harness (`:tools:daemon-harness`) needs the runtime classpath of this
...
// `:tools:daemon-harness` is plain `org.jetbrains.kotlin.jvm` so it can't directly consume an
...
// See `tools/daemon-harness/build.gradle.kts`'s `androidDaemonClasspath` consumer.
...
// `:tools:daemon-harness`'s `androidDaemonClasspath` configuration can wire up `dependsOn`
// without needing afterEvaluate semantics.
val writeDaemonClasspath by tasks.registering {
  description =
    "Resolves :renderer-android-daemon's debug runtime classpath + testFixtures into a text " +
...
afterEvaluate {
  ...
  // :renderer-android-daemon:test`), including AGP-generated R.jars for transitive AARs, the
  ...
  // generated id constants. Standalone `:renderer-android-daemon:test` gets it for free via
