 ...
 *   `:renderer-desktop-daemon/RenderEngineTest`.
 ...
 *   budget.
 *
 * Pixel-diff helper is inlined here rather than imported from `:tools:daemon-harness`'s
 * `PixelDiff` for the same reason as the desktop counterpart — `:renderer-android-daemon` ←
 * `:tools:daemon-harness` would invert the dependency graph.
 */
