 ...
 *
 * Mirrors `:renderer-desktop-daemon`'s
 ...
 * module's main source set so [DaemonMain] can mount it when spawned by `:tools:daemon-harness`'s
 ...
 *
 * **Why duplicated rather than promoted to `:renderer-daemon-core`.** Per DESIGN § 4 + § 7 the
