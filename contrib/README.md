...
- `ee.schimke.composeai:preview-data-api` — published wire-format DTOs (`PreviewResult`, `PreviewManifest`, `ExtensionPayload`, the v1 a11y mirror types). Consumed by any tool that reads compose-preview JSON output. See [`api/preview-data-api/`](../api/preview-data-api/).
- `ee.schimke.composeai:gradle-preview-driver` — Gradle Tooling-API render pipeline as a library. `GradlePreviewDriver.render(...)` returns `PreviewResult` lists with PNG sha256s populated, no CLI dependency. See [`api/gradle-preview-driver/`](../api/gradle-preview-driver/).
