...
/**
 * Shared host ↔ script state. The host ([ScriptCommand]) pre-populates [results] with the rendered
...
class ScriptState {
   ...
   * the script is evaluated, then read-only as far as the script body is concerned (the script sees
   * a snapshot, not a live mutable map).
   */
