...
const js_code = {
    ...
    'com.example.cmpwasmcatalog.fetchAsBase64' : (url, timeoutMs) => ((window.__cpPrefetchBuf && window.__cpPrefetchBuf[url]) ||
    ...
    'com.example.cmpwasmcatalog.fetchAsText' : (url, timeoutMs) => ((window.__cpPrefetchText && window.__cpPrefetchText[url]) ||
          fetch(url, { signal: AbortSignal.timeout(timeoutMs) })
            .then(function (r) { if (!r.ok) throw new Error('HTTP ' + r.status); return r.text(); })),
    'com.example.cmpwasmcatalog.consoleWarn' : (message) => console.warn(message),
    'com.example.cmpwasmcatalog.postToParent' : (message) => window.parent.postMessage(message, '*'),
    'com.example.cmpwasmcatalog.locationSearch' : () => window.location.search,
    'com.example.cmpwasmcatalog.locationHash' : () => window.location.hash.replace(/^#/, '')
}
