 ...
 * The `GOLDEN_*` vectors below are duplicated verbatim in `historyPaths.test.ts`; changing either
 * side alone makes the daemon write to one directory and the panel read from another, which
 * surfaces as a silently empty history drawer rather than a crash. Change the layout only with both
...
class HistoryPathsTest {
  ...
  // ---- Golden vectors (mirrored in compose-preview-vscode's `src/test/historyPaths.test.ts`) ----
