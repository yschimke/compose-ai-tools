...
plugins {
    id("ee.schimke.composeai.preview") version "0.2.1"
}
```
...
Download `compose-preview-0.2.1.tar.gz` (or `.zip`) from the
[v0.2.1 release](https://github.com/yschimke/compose-ai-tools/releases/tag/v0.2.1)
and put the `bin/` directory on your `PATH`:

```sh
curl -L -o compose-preview.tar.gz \
  https://github.com/yschimke/compose-ai-tools/releases/download/v0.2.1/compose-preview-0.2.1.tar.gz
tar -xzf compose-preview.tar.gz
export PATH="$PWD/compose-preview-0.2.1/bin:$PATH"
...
Download `compose-preview-0.2.1.vsix` from the
[v0.2.1 release](https://github.com/yschimke/compose-ai-tools/releases/tag/v0.2.1)
and install it:

```sh
code --install-extension compose-preview-0.2.1.vsix
```
...
`ee.schimke.composeai.preview` version `0.2.1` in your project as shown above.
