...
<!-- x-release-please-start-version -->
Download `compose-preview-0.8.3.tar.gz` (or `.zip`) from the
[v0.8.3 release](https://github.com/yschimke/compose-ai-tools/releases/tag/v0.8.2)
and put the `bin/` directory on your `PATH`:

```sh
curl -L -o compose-preview.tar.gz \
  https://github.com/yschimke/compose-ai-tools/releases/download/v0.8.3/compose-preview-0.8.2.tar.gz
tar -xzf compose-preview.tar.gz
export PATH="$PWD/compose-preview-0.8.3/bin:$PATH"
...
- uses: yschimke/compose-ai-tools/.github/actions/install@v0.8.3
  with:
    # Literal "0.8.3", "latest", or "catalog" (read from a Gradle
...
`ee.schimke.composeai.preview` version `0.8.3` <!-- x-release-please-version --> in your project as shown above.
