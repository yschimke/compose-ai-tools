...
Use `.github/workflows/codex-pr-review-reusable.yml` to run Codex PR review **only after** preview generation succeeds and with both code + visual context.
...
To avoid duplicate PR review comments, `.github/workflows/preview-comment.yml` is now kept as a legacy manual (`workflow_dispatch`) workflow.
