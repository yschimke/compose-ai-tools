...
abstract class GenerateRobolectricPropertiesTask : DefaultTask() {
  ...
  fun generate() {
    ...
    appTourDir.mkdirs()
    val appTourApplicationLine =
      if (useConsumerApplication.get() || appTourUseConsumerApplication.get()) {
        "# Application omitted — Robolectric falls back to the manifest-declared Application, " +
    ...
    appTourDir
      ...
      .writeText(
        """
            |# Generated by compose-ai-tools — the kind=ACTIVITY / kind=APP_TOUR render lane.
            |# Set composePreview.appTourUseConsumerApplication = false to render activities against
            |# a stub Application instead (they then skip the app's own onCreate, and any that need
            |# app-level DI will fail).
            |$appTourApplicationLine
            |$sdkLine
            |$graphicsLine
            |$shadowsLine
            |$instrumentedPackagesLine
    ...
    daemonDir.mkdirs()
    val daemonFile = daemonDir.resolve("robolectric.properties")
    val daemonBody =
      if (useConsumerApplication.get()) {
        """
        |# Generated by compose-ai-tools.
        ...
        """
        |# Generated by compose-ai-tools.
        |# Override the consumer's Application to a Robolectric-safe stub for daemon-driven preview
        |# rendering, so app-lifecycle init (DI, Firebase, WorkManager, …) never runs and crashes
        |# the render sandbox. sdk/graphicsMode are supplied by @Config/@GraphicsMode on the
        |# daemon's SandboxRunner; only the Application override needs this file because Robolectric
        |# 4.16 no longer merges the deprecated buildGlobalConfig override over the manifest.
        |application=android.app.Application
        |"""
          .trimMargin()
      }
    daemonFile.writeText(daemonBody)
  }
