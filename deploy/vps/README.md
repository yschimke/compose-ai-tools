...
`preview.coo.ee/m3-catalog/` shows). It needs a DNS record and one entry in
`catalogs.json`:
...
```json
"sites": [{ "host": "m3.preview.example.com", "system": "m3-catalog" }]
```

That one entry configures both processes. The app learns which catalog the `Host`
means, and the caddy container derives `SITE_DOMAINS` from the same file at start
([`../image/caddy-entrypoint.sh`](../image/caddy-entrypoint.sh), mounted by this
profile's compose file) — which is what makes Caddy match the hostname **and**
provision its Let's Encrypt certificate. There is nothing else to configure for
TLS, and no second list to keep in step; the `SITE_DOMAINS` variable remains only
for a name the app doesn't serve as a site.

On this profile `catalogs.json` is bind-mounted from the repo, so `git pull` plus
