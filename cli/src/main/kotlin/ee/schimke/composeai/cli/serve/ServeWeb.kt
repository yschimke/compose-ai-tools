  ...
  private fun viewerScript(): String =
    ...
    (function () {
      ...
      function openWasm() {
        ...
        // Frame parity: size the iframe to the snapshot's rendered box (before hiding it) so the
        // stage keeps its height across the switch — the in-browser app then scales its component to
        // fill that same frame, so toggling Wasm no longer makes the preview jump in size.
        if (img.clientHeight > 0) wasmFrame.style.height = img.clientHeight + "px";
      ...
      function closeWasm() {
        ...
        wasmFrame.style.removeProperty("height");
        img.hidden = false;
      }
