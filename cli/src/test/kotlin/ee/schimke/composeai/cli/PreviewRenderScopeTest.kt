...
class PreviewRenderScopeTest {
  ...
  // ---------- selectedPreviewIds: the request, for consumers that aren't the render (#3742) ----

  @Test
  fun `selectedPreviewIds returns every id when there is no request`() {
    val (_, manifest) = manifest(module("app"), "HomePreview", "SettingsPreview")

    assertEquals(
      listOf("HomePreview", "SettingsPreview"),
      PreviewRenderScope.selectedPreviewIds(manifest, exactId = null, filter = null),
    )
  }

  @Test
  fun `selectedPreviewIds narrows to the filtered ids`() {
    val (_, manifest) = manifest(module("app"), "HomePreview", "SettingsPreview", "HomeTilePreview")

    assertEquals(
      listOf("HomePreview", "HomeTilePreview"),
      PreviewRenderScope.selectedPreviewIds(manifest, exactId = null, filter = "home"),
    )
  }

  @Test
  fun `selectedPreviewIds answers the request even when it selects everything`() {
    // The case `Scope.renderedIds` cannot express: it is null here (the render declines to narrow,
    // nothing to save) *and* null when there is no request at all, so a per-preview consumer that
    // reads it can't tell "all of them" from "no filter" — see issue #3742.
    val (_, manifest) = manifest(module("app"), "HomePreview", "HomeSettingsPreview")
    val scope =
      PreviewRenderScope.forRequest(
        manifests = listOf(module("app") to manifest),
        exactId = null,
        filter = "Home",
      )
    ...
    assertNull(scope.renderedIds)
    assertEquals(
      listOf("HomePreview", "HomeSettingsPreview"),
      PreviewRenderScope.selectedPreviewIds(manifest, exactId = null, filter = "Home"),
    )
  }

  @Test
  fun `selectedPreviewIds matches expanded permutations but returns the declared id`() {
    val (_, manifest) = manifest(module("app"), "HomePreview", "SettingsPreview")

    // Same rule `forRequest` applies: the user names the id they saw in `show` output
    // (`HomePreview_dark`), and the daemon addresses the preview it was declared as.
    assertEquals(
      listOf("HomePreview"),
  ...
  @Test
  fun `selectedPreviewIds is empty when nothing matches`() {
    val (_, manifest) = manifest(module("app"), "HomePreview")

    assertEquals(
      emptyList(),
      PreviewRenderScope.selectedPreviewIds(manifest, exactId = "NoSuchPreview", filter = null),
    )
  }
