...
dependencies {
  ...
  //
  // No exclusion needed, and that is worth recording because an earlier draft of this line had one.
  // While `compose-preview-serve` still named the server's old `compose-preview-render-host`, that
  // artifact's classes were the same classes in the same package as `:render-host`, so both landed
  // on the test classpath — Gradle substitutes a published coordinate for a workspace project only
  // when `<group>:<projectName>` matches, and the old spelling did not match. Serve 3.0.0 names
  // `ee.schimke.composeai:render-host`, which does match, so the substitution happens by itself.
