...
|---|---|---|
| Kotlin BCV on `:gradle-plugin` / `:preview-annotations` | § 9 | No `.api` golden files, no CI check — a breaking API change ships silently. The `:rc-player-*` modules *are* gated (see § 9); these are not |
...
The 1.0 readiness punch list was [issue #798](https://github.com/yschimke/compose-ai-tools/issues/798); it covered feature completeness, not the enforcement above.
