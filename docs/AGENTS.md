...
- **File/IO goes through Okio + `Dispatchers.IO`, not `java.io.File` / `readText()` / `writeText()`.** All non-Gradle production code funnels disk access through [`:common-io`](../common/io) — `SystemFileSystem` (an Okio `FileSystem`) plus the suspend helpers in [`SuspendIo.kt`](../common/io/src/main/kotlin/ee/schimke/composeai/io/SuspendIo.kt) (`readUtf8` / `writeUtf8` / `read{Byte,ByteString}s` / `writeBytes` / `readJson` / `writeJson` / `createTempFile` / `fileIo { … }`). Every helper hops onto `Dispatchers.IO`, so **blocking disk access never runs on a computation/UI dispatcher** — push `suspend` up the call chain to the nearest real entry point (CLI `main` / Clikt `run()`, a JSON-RPC handler, a Compose `LaunchedEffect`) and terminate it there with `runBlocking` rather than blocking mid-chain. Paths are `okio.Path` (`"x".toPath()`, `dir / "file"`); reach for `java.io.File` only at a **hard third-party boundary** that demands it — the Gradle Tooling API (`GradleConnector.forProjectDirectory`), `java.util.zip.ZipFile`, `URLClassLoader` (`file:` URLs), `ImageIO`, `ProcessBuilder` — and bridge there with Okio's `Path.toFile()` / `File.toOkioPath()`, keeping the `File` local to that call. The **Gradle plugin / `build-logic` are exempt** (they live in Gradle's own `java.io.File` world). Public signatures on published modules take `okio.Path` and are `suspend` too — this was a deliberate breaking change (see the migration note below); don't reintroduce `File` into a public API to dodge the suspend cascade. Changes to plugin code must resolve classpaths/JVM args at configuration time via lazy providers — never call `.files` inside a task action or touch `project.*` at execution time. IP additionally forbids cross-project access: no `allprojects {}` / `subprojects {}`, no `project(":other")`, no `gradle.taskGraph.whenReady { allTasks }`. Shared per-project setup goes in a `build-logic` convention plugin applied per-module, not pulled in from the root.
