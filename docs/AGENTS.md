...
- **Toolchain:** Java 17, Kotlin 2.3.21, Gradle 9.4.1+, AGP 9.2.1, CMP 1.10.3. Always use the bundled `./gradlew` wrapper. Don't loosen the toolchain to a newer JDK to avoid the install — AGP / Robolectric still target 17, and bumping silently produces classes-vs-resources skew on the consumer's unit-test classpath.
