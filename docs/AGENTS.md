...
- **Bringing up a fresh sandbox.** When `./gradlew` fails with "Unable to download toolchain": `sudo apt-get install -y openjdk-17-jdk-headless` — Gradle's auto-detection picks it up from `/usr/lib/jvm/`. When an Android-flavoured sample then fails with "SDK location not found", install the SDK manually (no apt package covers `compileSdk = 36`):
  ...
  ```
  `local.properties` is `.gitignore`d; CI uses `ANDROID_HOME` instead. Re-running `:sample-wear:renderAllPreviews` after this end-to-end takes ~3 min cold. Or run [`scripts/install.sh --android-sdk`](../scripts/install.sh), which automates the same steps idempotently and writes `ANDROID_HOME` to `$CLAUDE_ENV_FILE` when it detects a Claude Code cloud session.
- **Claude Code on the web.** Put `bash <(curl -fsSL https://raw.githubusercontent.com/yschimke/compose-ai-tools/main/scripts/install.sh) --android-sdk` in the cloud environment's **Setup script** field. The script's filesystem changes (JDK + SDK + the compose-preview CLI) are baked into the cached snapshot Anthropic reuses for later sessions in that environment, so subsequent cold starts skip the ~30 min SDK install. The environment must use **Custom** network access with `dl.google.com` added to the allowlist — the default Trusted list covers `developer.android.com` (the docs domain) but not the SDK CDN. See [code.claude.com/docs/en/claude-code-on-the-web](https://code.claude.com/docs/en/claude-code-on-the-web) for the snapshot/cache mechanics.
