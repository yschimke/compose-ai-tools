...
- **File/IO goes through Okio, not `java.io.File` / `readText()` / `writeText()`.** All non-Gradle production code funnels disk access through [`:common-io`](../common/io) — `SystemFileSystem` (an Okio `FileSystem`) plus Okio's own blocking `read { … }` / `write { … }` / `source` / `sink`. Paths are `okio.Path` (`"x".toPath()`, `dir / "file"`). `:common-io` is **deliberately coroutines-free**: it has to stay loadable on the **render subprocess classpath** without dragging a `kotlinx-coroutines` version skew onto it (a skew there breaks Compose rendering with `NoSuchMethodError` — see [docs/RENDERER_COMPATIBILITY.md](RENDERER_COMPATIBILITY.md)). The `suspend` + `Dispatchers.IO` wrappers (`readUtf8` / `writeUtf8` / `read{Bytes,ByteString}` / `writeBytes` / `readJson` / `writeJson` / `createTempFile` / `fileIo { … }`) live in the separate [`:common-io-suspend`](../common/io-suspend) module (same `ee.schimke.composeai.io` package) — **only depend on it from genuinely async code** (e.g. `:bundle-viewer`'s load runs in a Compose `LaunchedEffect`); everything else stays synchronous on `:common-io`. Reach for `java.io.File` only at a **hard third-party boundary** that demands it — the Gradle Tooling API (`GradleConnector.forProjectDirectory`, the serialized `PreviewModule` — `okio.Path` isn't `Serializable`), `java.util.zip.ZipFile` (or use `FileSystem.openZip`), `URLClassLoader` (`file:` URLs), `ImageIO` (bridge via `source().inputStream()` / `sink().outputStream()`), `RandomAccessFile`, `ProcessBuilder` — and bridge there with `Path.toFile()` / `file.path.toPath()`, keeping the `File` local to that call. **Exempt** (stay `java.io.File` / `java.nio`): the Gradle plugin + `build-logic` (Gradle's own world), the CodeQL-recognized `java.nio.Path` zip-slip containment checks, the BTA in-process compiler's `java.nio.Path` source list, and the `java.nio.Path`-native, `ATOMIC_MOVE`-based history stores (`LocalFsHistorySource` / `GitRefHistorySource`). Changes to plugin code must resolve classpaths/JVM args at configuration time via lazy providers — never call `.files` inside a task action or touch `project.*` at execution time. IP additionally forbids cross-project access: no `allprojects {}` / `subprojects {}`, no `project(":other")`, no `gradle.taskGraph.whenReady { allTasks }`. Shared per-project setup goes in a `build-logic` convention plugin applied per-module, not pulled in from the root.
