...
- The checked-in manifest exactly matches all 172 public integer entries in AndroidX alpha16
  ...
  test prevents drift. The current disposition is 72 implemented, 9 parse-only, 81 pending,
...
- Cluster 2 includes AndroidX-compatible path creation, append, combine, tween, expression-generated
  ...
  These seven operations and the existing layout-bound `DrawContent` and `LayoutCanvasContent`
  remain parse-only—and therefore excluded from the Wasm profile—until the component measurement
  and rendering interpreter exists.
