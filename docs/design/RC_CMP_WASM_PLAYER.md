...
- The checked-in manifest exactly matches all 172 public integer entries in AndroidX alpha16
  ...
  test prevents drift. The current disposition is 72 implemented, 2 parse-only, 88 pending,
...
- Cluster 2 includes AndroidX-compatible path creation, append, combine, tween, expression-generated
  ...
  The two remaining
  parse-only operations are layout-bound `DrawContent` and `LayoutCanvasContent`; they remain
  explicitly non-renderable until the component interpreter exists.
