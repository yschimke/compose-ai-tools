...
- The checked-in manifest exactly matches all 172 public integer entries in AndroidX alpha16
  ...
  test prevents drift. The current disposition is 81 implemented, 3 parse-only, 78 pending,
...
- Cluster 2 includes AndroidX-compatible path creation, append, combine, tween, expression-generated
  ...
  and AndroidX's additive `spacedBy` rule. Fit-box, `DrawContent`, and `LayoutCanvasContent` remain
  parse-only—and therefore excluded from the CMP profile—until their rendering semantics land.
