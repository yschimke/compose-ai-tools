...
`:samples:design-catalog-wear-m3` used one until it was found to be publishing a
resting button as its `pressed` variant. `WearFocusedPressPixelTest` is the guard
— the pressed capture must differ from *both* the focused and the resting one,
which catches a press that never happened and one that happened but did not
paint.
