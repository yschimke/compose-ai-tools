...
Status: **architecture, decided in outline; two things open** (2026-09). Written
after reading the `design-parity` `figma-plugin` source rather than this
repository's description of it, which is what turned up how close the two halves
already are.
...
1. **Codegen runs server-side, not in the plugin.** The plugin would POST a
   `Screen` and receive Kotlin. Porting `ScreenCodegen` to TypeScript would create
   a second implementation of the one thing that must not drift — the mapping from
   a document to the code it claims to be.
2. **`:screen-model` gets published.** It is JVM + wasmJs and currently
   unpublished, deliberately ("a coordinate is permanent"). A server consuming it
   by coordinate is what forces the decision. Do it when the node shape stops
   moving — the two open questions below are the ones still moving it.
