...
3. **Merge the release PR.** On the next `release-please.yml` run (fires immediately on the merge commit), it creates the `vX.Y.Z` tag + a **draft** GitHub Release, then starts two independent paths from that tag. `release.yml` builds and uploads the core artifacts and publishes the Maven deployment; a `finalize-release` job verifies the required assets, un-drafts the Release, and marks it `latest` without waiting for Maven Central's CDN to propagate. A separate readiness job resolves the CLI's exact auto-injected plugin classpath from public Maven Central and then attaches a `compose-preview-maven-ready-<version>.json` marker. The bootstrap installer withholds the new CLI until that marker is downloadable, preventing an update that fails on its first Gradle-backed command; the `install` action's `version: latest` gates on the same marker, so CI dispatched inside the publication window installs the previous release rather than asking Gradle for a plugin that is not on the Plugin Portal yet (issues #5034, #5051). Once the release is finalized, the `release-please.yml` run **ends** — the delivery-branch renders no longer hang off it. `post-release-design-artifacts.yml` picks that completed run up via `workflow_run`, confirms the newest published release tag points at the commit it built, and rebuilds the `design-artifacts/<system>` delivery branches against the just-released preview-runtime coordinates in a run of its own; those jobs own their own Maven Central readiness wait. If a required core asset is missing the release **stays a draft** so it's never half-published — re-run `release.yml` for the tag, then re-run the finalize step.
   ...
   > **A push landing during the release can't duplicate the release PR.** release-please baselines on the latest *published* Release, so for the ~20 minutes a release spends as a draft, the version `main` already carries is invisible to it — any PR-half invocation in that window re-proposes the version being released. The release run avoids this by construction (`skip-github-pull-request`, then `reconcile-release-pr` after publish), but an ordinary `main` push holds a separate concurrency slot and can run the PR half straight through the cut: that is how the duplicate `chore(main): release 1.42.0` PR #4645 appeared. [`release-pr-race-guard.sh`](../.github/scripts/release-pr-race-guard.sh) now asks "is a release in flight?" before and after that invocation — skipping it in the ordinary case, and closing the duplicate when the merge lands mid-invocation. It fails open: a release left unpublished for over two hours reads as stranded, not in flight, so a wedged release can never freeze the release PR.
   >
   ...
   > **Only the release-PR merge run cuts the release.** `release-please.yml` runs on *every* push to `main`, and its first invocation used to be able to create the GitHub Release on any of them. Two pushes seconds apart therefore raced for the same pending release PR: on 2026-09-05 both went for v1.78.0, the winner stripped `autorelease: pending` from #5148, and the loser died on `Label does not exist` — after it had already cut the draft. The action sets its outputs last, so `release_created` came back empty and the tag step, `release.yml` and `finalize-release` all skipped, leaving v1.78.0 a stranded draft. It escaped only because the next push to `main` happened to re-cut it. Now the release half is gated on `RELEASE_MERGE` — the push whose head commit is `chore(main): release …`, or a manual dispatch — so an ordinary push's invocation is a no-op and there is nothing to race. Recovery is deliberate rather than accidental: see the sweeper below.

   > **A draft that nothing is publishing gets swept up within the hour.** The whole chain hangs off `release_created == 'true'`, so any throw between cutting the draft and the action returning strands it — `Error adding to tree` did it to 0.16.34 (draft *and* untagged, fixed by hand), `Label does not exist` to v1.78.0. [`release-draft-sweeper.yml`](../.github/workflows/release-draft-sweeper.yml) is the backstop: hourly, and only when no release run is in flight, it looks for a draft older than the release window and finishes it — creating the missing tag (at the draft's own commit; never a branch tip), dispatching `release.yml` when the assets never built, and publishing it once they are all attached *and* the Gradle plugin resolves from Maven Central. It shares `finalize-release`'s asset list ([`required-release-assets.sh`](../.github/scripts/required-release-assets.sh)) so the out-of-band gate can't be looser than the in-chain one, and it goes red after 24 hours on a draft it cannot finish, because a scheduled job nobody watches has no other way to ask for a human. Rules and refusals are pinned by [`test-sweep-stranded-releases.sh`](../.github/scripts/test-sweep-stranded-releases.sh).
...
plugins {
    id("ee.schimke.composeai.preview") version "1.82.0"
}
```
...
curl -L -o compose-preview.tar.gz \
    https://github.com/yschimke/compose-ai-tools/releases/latest/download/compose-preview-1.82.0.tar.gz
tar xzf compose-preview.tar.gz
./compose-preview-1.82.0/bin/compose-preview list
```
<!-- x-release-please-end -->
...
curl -L -o compose-preview-mcp.tar.gz \
    https://github.com/yschimke/compose-ai-tools/releases/latest/download/compose-preview-mcp-1.82.0.tar.gz
tar xzf compose-preview-mcp.tar.gz
./compose-preview-mcp-1.82.0/bin/compose-preview-mcp
```
<!-- x-release-please-end -->
