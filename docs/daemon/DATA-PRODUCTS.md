...
| `text/strings`             | default | low | Text drawn on screen with locale, fontScale, bounds. **Android daemon implementation:** v1 prefers `GetTextLayoutResult` literal text, also emits semantics text/labels so visible text and accessibility text can differ; resource entry names belong to `resources/used`. |
