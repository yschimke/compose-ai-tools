...
The daemon is available by default in any project that applies the Compose Preview Gradle plugin; opt out with `composePreview { daemon { disabled = true } }`. The Gradle `renderPreviews` task remains the always-available fallback and the CI-canonical render path.
