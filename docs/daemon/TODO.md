...
* **S8 / metrics round-trip** — `RenderFinishedParams.metrics` is typed as the structured `RenderMetrics?` (B2.3 shape), not a free-form `Map<String, Long>`; `renderFinishedFromResult` discards `result.metrics` regardless. The v1 task brief forbids widening `:daemon:core`, so S8 asserts the FakeHost loader path (re-parses `<previewId>.metrics.json` and asserts the expected map) **plus** that `renderFinished.metrics` is null on the wire today (gap-as-regression-test).
