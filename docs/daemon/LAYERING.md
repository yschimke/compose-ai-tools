...
```
   ...
   │   :tools:daemon-mcp — separate module, separate process by   │
...
- **Same-process mode is allowed but explicit.** The user has noted
  ...
  - A deliberate launch mode flag (`--embed-daemon` or equivalent),
    not the default.
  - Implemented with the *same* `:tools:daemon-mcp` ↔ daemon
...
- Delete `:tools:daemon-mcp` and its task wiring.
...
- A user who opts into the daemon for VS Code never compiles the
  MCP module.
- An agent author who wants MCP gets it via `:tools:daemon-mcp` —
