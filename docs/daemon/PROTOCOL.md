...
An unknown row fails the render rather than silently falling back to value 0, and the failure lists the provider's actual rows — with no enumeration RPC yet, asking for one past the end (`PARAM_999`) is how a client discovers what a provider offers. Cost is bounded: an unaddressed render still enumerates with `take(1)`, an index request stops at `n + 1`, and a label request is capped by `PreviewParameterSupport.MAX_ROW_SCAN`, so an infinite `generateSequence` provider is never driven to exhaustion.
