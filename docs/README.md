...
- [design/CATALOG_ONBOARDING.md](design/CATALOG_ONBOARDING.md) — **plan**: onboarding a GitHub project into the preview server from its URL alone (#4789). Splits the one admin token into three consents — repo ownership proven per request, the operator's agreement to host, and trust (which gates server-side execution and is never automatic) — and phases the build around the fact that the server never builds the repo: its CI publishes a `design-artifacts/<system>` branch and the box agrees to serve it.
