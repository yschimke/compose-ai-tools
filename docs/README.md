...
- [RENDER_ASSERTIONS.md](RENDER_ASSERTIONS.md) — declaring properties a render must hold to (a font, a required role) and checking them against the data-product sidecars a render already writes.
...
The `design/` tree holds the specs the code actually depends on:
...
- [design/DEVICE_CAPTURE.md](design/DEVICE_CAPTURE.md) — **proposal**: turning a screen in a *running* app into the same data products the render lane emits for a `@Preview`. Two tiers — an in-app producer taken as a `debugImplementation` (Tier 2), and a Layout Inspector `.li` snapshot importer for an app that cannot take one (Tier 1) — on the finding that the producers are already pure functions of a `SemanticsNode` root plus slot tables, density and font scale, all of which a live process has. Covers the two producers that need module surgery before an app can carry them (a connector that drags FontBox, and an a11y core that carries Robolectric), the four assumptions the render lane gets free and a device does not — one frame, one coordinate space, one Compose root, a durable home — the internal-API composable wrapper `LocalInspectionTables` actually needs, `testTagsAsResourceId`, and why a capture is a reference and not a builder document.
...
- [design/CROSS_REPO_RELEASES.md](design/CROSS_REPO_RELEASES.md) — the order to release the five repositories in, and why it is strictly downhill. The graph and the per-repository pins, the wait-for-Central step between each cut, and the friction worth knowing about in advance: why an external sweep and an inter-repo bump must not share a PR, why Renovate ceilings do not propagate when a module moves (and why green CI is not evidence one held), and why a coordinate's group says nothing about which repository owns it.
- [design/RELEASE_TRAINS.md](design/RELEASE_TRAINS.md) — **measurement + proposal**: this repository publishes 94 Maven artifacts on a line that releases ~6 times a day, and 96.7% of what it uploads is a version-bumped rebuild of identical code (117 module-changes against 3,572 module-publications over 38 releases). Measures each lever: a change-detection guard on the publish (−15.8%, implemented in reporting mode), splitting the publish into five cadence-based trains (−45.7%), and why skipping is unsafe until the CLI names the Maven version it was built against rather than assuming its own.
- [design/REMOTE_COMPOSE_JSON.md](design/REMOTE_COMPOSE_JSON.md) — **normative**: the Remote Compose JSON format is *two* formats, and every mistake in this area starts by conflating them. Authoring JSON is a source language AndroidX's `RemoteComposeJsonParser` compiles to `.rc`; document JSON is this repository's operation-level projection of a compiled document, for reading and diffing, which nothing reads back. Covers why the dump rides AndroidX's own `Serializable` hook rather than a hand-written binary reader (a reader against a format with no checksums does not break when it drifts — it keeps parsing, wrongly), why ids travelling as NaN payloads force every non-finite float to be a string, why the codec is deliberately *not* in the Gradle plugin, and the trap where `parseToByteBuffer("{}")` succeeds and returns a playable document that renders blank with nothing anywhere reporting an error.
...
- [design/DAEMON_LAUNCH_SCHEMA.md](design/DAEMON_LAUNCH_SCHEMA.md) — **normative**: the `daemon-launch.json` contract and the `checkDaemonLaunchSchema` gate that holds its four representations, in two languages across three Gradle builds, to one shape. What each of the ten rules catches, the allowlist discipline, and the two reachability notes that decide whether the gate actually runs on a PR.
- [design/DESIGN_CATALOGS.md](design/DESIGN_CATALOGS.md) — the code-led sticker-sheet catalog system.
- [design/OUTPUT_REPOSITORIES.md](design/OUTPUT_REPOSITORIES.md) — **measurement + proposal**:
  generated branches dominate the catalog repositories (1.20 of 1.24 GiB in Wear), the
  source/output identity and credential split needed to move them safely, the `compose-m3` pilot,
  and the bounded-retention policy required so output repositories do not grow forever.
...
- [design/UI_BUILDER_CATALOG_POLICY.md](design/UI_BUILDER_CATALOG_POLICY.md) — **in progress**: what a catalog repository authors so a UI builder can serve it without being written into the server — `@BuilderComponent` beside `@CatalogComponent` for per-component policy, `ui-builder.policy.json` beside `catalog.spec.json` for the platform word, the measured frame and the structural code templates, and one generated `ui-builder.json` published beside `components.json`. Generated by the discovery task rather than the design-artifacts pipeline, so a local `compose-preview-server ui` reads the same file a deployment does.
