...
- [design/COMPONENT_PARITY_WORKFLOW.md](design/COMPONENT_PARITY_WORKFLOW.md) — **proposal**: turning parity reporting into an iterative loop — a stable component/preview/reference locator, a published GitHub issue index (`parity/issues.json`), and issue-linked *scoped* acceptance of one known difference that still detects everything else. Investigation of the existing surfaces + phased plan; no code yet ([#3680](https://github.com/yschimke/compose-ai-tools/issues/3680)).
...
- [design/RC_EMBEDDED_PLAYER_UPSTREAM_FIXES.md](design/RC_EMBEDDED_PLAYER_UPSTREAM_FIXES.md) — **work order**: the nine fixes we carry on the vendored embedded player that upstream has not taken, written to be handed to an agent working in an androidx checkout — landing site, current upstream code, the change, the test to add, and the AOSP Gerrit route (`compose:remote` does not accept GitHub PRs). Outbound companion to the audit below; landing these is what lets the Android lane drop the vendored sources.
- [design/RC_EMBEDDED_PLAYER_UPSTREAM_SYNC.md](design/RC_EMBEDDED_PLAYER_UPSTREAM_SYNC.md) — **audit + plan**: every fix we carry on the vendored AndroidX embedded player (`third_party/rc-embedded-player`) against a tree diff of current `androidx-main` — which of our deltas upstream has since adopted, which of its new fixes to take, what a refresh will actually cost, and why the vendoring rationale expires once the next `compose-remote` alpha ships the player as a published artifact.
