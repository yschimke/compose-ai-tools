...
- The [`No Agent Attribution`](../.github/workflows/no-agent-attribution.yml)
  gate bans agent **identities** (`@anthropic.com` / `@openai.com` emails, or
  an agent **name** like `Claude` on the author/committer) and agent
  `Co-authored-by:` trailers. GitHub App bot accounts (`…[bot]`) are exempt, so
  a `claude[bot] <claude[bot]@users.noreply.github.com>` identity on a
  **branch** commit passes — and squash-merging from the PR title + body keeps
  `main` authored by the human who opens/merges the PR. The
  `_Generated by [Claude Code]_` footer is explicitly allowed (it links the
  session; it doesn't claim authorship).
