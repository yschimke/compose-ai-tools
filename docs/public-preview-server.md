...
[`/{system}/api/uses`](../cli/serve/src/main/kotlin/ee/schimke/composeai/cli/serve/ServeHttpServer.kt)
...
[`PreviewUsageIndex`](../cli/serve/src/main/kotlin/ee/schimke/composeai/cli/serve/PreviewUsageIndex.kt)
...
## The parity loop: report, accept, verify, close

The dashboard above says *how far apart* the two sides are. This section is about what a catalog
owner does with one difference — from the moment somebody sees it to the moment the issue closes and
the record that described it is deleted. The design record, with the reasoning behind every rule
here, is
[`docs/design/COMPONENT_PARITY_WORKFLOW.md`](design/COMPONENT_PARITY_WORKFLOW.md); this is the
shorter, operational half.

### 1. File it from the comparison

Open the focused **Reference / Diff / Actual** page for the preview
(`/<system>/compare/<previewId>?reference=<referenceId>`) and use **report a catalog issue**. The
...
wants to say goes above or below it. A hand-written issue needs a block pasted in — the preview
server's URL is a reproduction link, not the identity, because hosts, tokens and branches change.

Filing a report **accepts nothing**. It records a difference; whether that difference is tolerated is
a separate, committed decision (step 3).

### 2. Label it for triage

Two small label vocabularies, both low-cardinality on purpose:

| Label | Means |
| --- | --- |
| `area:spec` | the design reference is wrong or out of date |
| `area:component` | the code doesn't match a correct spec |
| `area:preview` | the fixture is wrong — wrong state, theme, size, knob |
| `area:renderer` | the render pipeline drew it wrong |
...
The last two are also the report form's own question — **"Where does it belong?"**, beside the
Summary field on the viewer, the focused comparison and the comparison wall. Whatever is chosen
there rides to GitHub as the `labels` parameter, so the label is applied as the issue is opened, and
the body's `**Where it belongs:**` line says the same thing in prose for a repository that has no
such label to apply.

Its **default answers nothing and labels nothing**: "needs investigating — not sure yet" says so in
the body and leaves the label list alone. That is deliberate on both counts. Not knowing is the
honest answer when the reporter cannot tell, and the point of asking on the comparison is that the
person looking at both pictures is best placed to say — not that they must guess. And a `parity:`
label attached to every report by default would make the index's "labelled as parity work but
carries no locator block" warning fire for reports filed exactly as designed, which is the one place
that warning has to keep meaning something.

The question is not asked on the pages that file a report about a *page* — the landing grid, the
pages index, a design page, the motion browser. There is no comparison there for "upstream or ours"
...
component labels would make the tracker unusable while answering a question the locator already
answers exactly.

Issues reach the component pages through `parity/issues.json` on the delivery branch, regenerated by
a small workflow in the catalog repo on every issue event. It commits one path and needs no
re-render, so a relabelled issue reaches a serving host within one refresh tick. Set-up is in
[Publishing the issue index](#publishing-the-issue-index) below.

### 3. Accept one difference — narrowly, and against an issue

Some differences are deliberate and are going to stay for a while. An **acceptance** records exactly
that difference so it stops reading as an untriaged regression, without turning into an ignore
rectangle. It lives in the **source** repo, beside `design-map.json`:
...
.design-parity/
  ...
  known-differences/
    m3-iconbutton-tonal-glyph/
      mask.png                      # binary mask, canonical plane
      accepted-candidate.png        # the accepted crop, same plane
```

Each record carries a stable `id` (which is also its directory name), a **mandatory `issue` URL**,
the locator scope it applies to (`system` / `component` / `previewId` / `referenceId` / `variant`),
the `plane` the mask was authored against, three hashes (`referenceSha256`, `maskSha256`,
`acceptedCandidateSha256`), a `candidateTolerance`, and optionally an `element` selector with its
authoring-time bounds and its own `tolerance`.

Things that refuse a record outright, so they are worth getting right first time:

- **`mask.png` must be 8-bit greyscale, colour type 0, no `tRNS`, samples strictly `0` or `255`.**
  The encoding is checked in the PNG header, not inferred from the samples, because a browser decode
  normalises everything to RGBA and would not see the difference. An indexed or RGBA mask is
  `mask-encoding-invalid` even when its samples look binary.
- **All three hashes must match the committed bytes.** An edited mask with an unchanged record would
  silently widen what is suppressed, so a mismatch is a refusal, not a degrade-to-normal.
- **`0 ≤ candidateTolerance ≤ 8`** (8-bit channel distance; `2` is a sensible value) and
  **`0.0 ≤ element.tolerance ≤ 0.25`** (`0.1` is sensible). Out of range is
  `tolerance-out-of-range`.

**A tolerance that needs to be large is evidence the acceptance is wrong.** Both numbers exist for
resample and round-trip slack; widened far enough, either one turns the acceptance into exactly the
blanket ignore the model exists to prevent — a large `candidateTolerance` lets any future candidate
match the accepted crop, and a large `element.tolerance` lets the element drift away from the region
the mask covers. If the difference cannot be captured within those bounds, the honest move is a
smaller mask, a different element, or no acceptance.

What an acceptance does **not** do is hide the raw finding. Reports carry `raw`, `unaccepted` and
`accepted` as three independent numbers, and the comparison page shows all three: `raw` is the pair
as though nothing had been accepted and is never suppressed. Pixels outside the mask are compared
normally; pixels inside are compared against the recorded accepted crop, so a *new* difference in
the accepted region still shows up.

There is currently **no UI for authoring a mask** — a human writes the record and produces the two
PNGs. That is tolerable while acceptances are rare and deliberate, which is what they should be.

### 4. Read the statuses, and act on them

Every acceptance gets a verdict per comparison, shown in the band under the focused comparison, and a
catalog-wide roster on the dashboard's **Known differences** panel. The two answer different
questions and neither replaces the other: the band judges *this pair's pixels*; the panel walks the
whole document with no comparison at all, which is the only way to see an acceptance that no
comparison scopes in any more.

| Status | Means | Do |
| --- | --- | --- |
| `valid` | still matches what was recorded | nothing |
...
half-finished. That join is **positive evidence only**: an acceptance whose issue the index does not
mention stays *unknown* and is never reported as stale, because the index is fail-soft, capped, and
can lag behind a regeneration.

### 5. Close the loop in one change

When a difference is really gone, two things have to happen: the record and its directory are
deleted, and the issue is closed. They happen in **one pull request**, with a closing keyword in the
description, so the merge does both atomically. Deleting first would leave the issue as the only
record of a difference nobody can describe any more; closing first leaves any sibling acceptance
briefly stale against a closed issue.

One restriction on the automated half: "every acceptance linked to this issue" is not answerable from
a run that reads a single `known-differences.json`. Where ownership cannot be established, the run
deletes its own resolved records and **omits** the closing keyword, leaving the close to whoever can
see the other referencing documents. Deleting locally is always safe; closing is the half that needs
knowledge one run does not have.

### Publishing the issue index

The index workflow lives in the **catalog** repo and calls
[`parity-issues-reusable.yml`](../.github/workflows/parity-issues-reusable.yml) in this one. Two
things about its credentials are worth stating plainly, because getting either wrong looks exactly
like "no issues changed":

- **The caller must grant `contents: write` and `issues: read`.** A called workflow cannot widen the
  permissions its caller hands it, so the job block in the catalog repo — not the reusable workflow —
  is where those live. `contents: write` commits the one path onto `design-artifacts/<system>`;
  `issues: read` is what the producer reads.
- **A source repo other than the one running the workflow needs a token that can read it.** The
  default `github.token` is scoped to the repository it runs in, so an extra entry in `repositories`
  is read with `ISSUES_TOKEN` when one is configured. Point it at a credential whose installation
  actually covers every repository listed — a 403 from one of them is indistinguishable from a repo
  with no parity issues, and the index publishes happily without those rows.

A weekly `schedule:` run backs the issue-event trigger up, so a missed webhook costs at most a week
of staleness rather than being permanent.
...
**Known formats** ([`ServeDocFormats`](../cli/serve/src/main/kotlin/ee/schimke/composeai/cli/serve/ServeDocFormats.kt)) —
...
- The caller sends `Authorization: Bearer <github-token>`, and the server asks **GitHub** who that is
  ...
  bar is the same one the playground applies ([`GitHubOAuthVerifier`](../cli/serve/src/main/kotlin/ee/schimke/composeai/cli/serve/ServeGithubAuth.kt)):
...
**Raster only** ([`ServeImageFormats`](../cli/serve/src/main/kotlin/ee/schimke/composeai/cli/serve/ServeImageFormats.kt)) —
...
1. **From a carried executable bundle (`liveBundle`) — no build (preferred, and now the default).**
   ...
   **snapshot** build, and a `-SNAPSHOT` coordinate was doubly unreachable — not declared anywhere
   ...
   ([`ServeBundleDaemon.materialize`](../cli/serve/src/main/kotlin/ee/schimke/composeai/cli/serve/ServeBundleDaemon.kt)):
