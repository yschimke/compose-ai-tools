...
3. **Daemon profile.** The daemon's a11y extension is registered
   unconditionally — no sysprop, no Gradle flag. The existing real-Gradle
   test uses `RealGradleApi`; the wear module's `daemon-launch.json` is
   regenerated by the plugin and the test inherits the always-on path.
