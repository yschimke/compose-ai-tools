...
3. **Daemon profile.** The daemon's a11y data-product registry is registered
   unconditionally; clients flip it on per-preview via `data/subscribe`. The
   standalone Gradle render task no longer runs ATF — a11y is daemon-only —
   so the test exercises the chip → `setDataProductSubscription` → daemon
   path end-to-end and no Gradle property is involved.
