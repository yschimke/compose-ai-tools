...
| **Robolectric** | stable `4.16.1`, snapshot `4.17-SNAPSHOT` | The stable line tops out at API 36. Snapshots from upstream `master` (see upstream commit [`0e89b68`](https://github.com/robolectric/robolectric/commit/0e89b684f5871ae6c65f973bc34aa022ec9a541e)) carry API 37 fixes ahead of the next release. The matrix probes both so the doc tells consumers whether a `compileSdk = 37` project is rescuable today. The snapshot override is exposed in the sample as `-Pcomposeai.matrix.robolectricVersion=<version>` and pulls from the Sonatype snapshots repo. |
...
| 17 | 37 | 37 | 24 | auto | 4.16.1 | ❌ fail | Auto-detect resolves to `sdk=37`; stable Robolectric has no API 37 jar AND JDK 17 wouldn't accept it anyway. Sandbox bootstrap throws "API level 37 is not available". |
...
| 21 | 37 | 37 | 24 | auto | 4.16.1 | ❌ fail | Auto-detect resolves to `sdk=37`; stable Robolectric has no `android-all-instrumented-37.jar`. Sandbox bootstrap throws "API level 37 is not available". Use the snapshot probe below or wait for the next Robolectric release. |
| 21 | 37 | 37 | 36 | auto | 4.16.1 | ❌ fail | Same shape as the row above; minSdk is moot when the sandbox can't even bootstrap. |
| 21 | 37 | 37 | 24 | auto | 4.17-SNAPSHOT | ✅ pass | **Snapshot probe.** Upstream's API 37 fixes ([commit `0e89b68`](https://github.com/robolectric/robolectric/commit/0e89b684f5871ae6c65f973bc34aa022ec9a541e)) let the sandbox bootstrap at SDK 37. PNG shows `SDK_INT = 37`. Drops back to ❌ if the snapshot regresses or stops shipping API 37. |
