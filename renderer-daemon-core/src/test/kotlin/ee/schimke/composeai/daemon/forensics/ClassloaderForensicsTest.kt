 ...
 * `:renderer-android-daemon`'s `ClassloaderForensicsDaemonTest` (Configuration B). This test only
...
class ClassloaderForensicsTest {
  ...
  fun rendererAgnosticSurfaceCarriesNoComposeOrRobolectricImports() {
    ...
    // classpath. If this test runs at all on `:renderer-daemon-core` (which has no
