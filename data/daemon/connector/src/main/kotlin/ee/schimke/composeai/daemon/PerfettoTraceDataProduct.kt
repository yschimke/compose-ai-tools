...
typealias TraceEvent = ee.schimke.composeai.data.daemon.TraceEvent

typealias TraceMetadata = ee.schimke.composeai.data.daemon.TraceMetadata

typealias TracePayload = ee.schimke.composeai.data.daemon.TracePayload

object PerfettoTraceDataProducer {
  const val KIND: String = ee.schimke.composeai.data.daemon.PerfettoTraceDataProducer.KIND
  const val SCHEMA_VERSION: Int =
    ee.schimke.composeai.data.daemon.PerfettoTraceDataProducer.SCHEMA_VERSION
  const val FILE: String = ee.schimke.composeai.data.daemon.PerfettoTraceDataProducer.FILE
  const val ENABLED_PROP: String =
    ee.schimke.composeai.data.daemon.PerfettoTraceDataProducer.ENABLED_PROP

  fun enabled(): Boolean = ee.schimke.composeai.data.daemon.PerfettoTraceDataProducer.enabled()

  fun recorder(previewId: String, backend: String, enabled: Boolean = enabled()): Recorder =
    Recorder(
      ee.schimke.composeai.data.daemon.PerfettoTraceDataProducer.recorder(
        previewId = previewId,
        backend = backend,
        enabled = enabled,
      )
  ...
  fun writeArtifacts(rootDir: File, previewId: String, trace: TracePayload) {
    ee.schimke.composeai.data.daemon.PerfettoTraceDataProducer.writeArtifacts(
      rootDir = rootDir,
      previewId = previewId,
      trace = trace,
    )
  ...
  class Recorder(
    private val delegate: ee.schimke.composeai.data.daemon.PerfettoTraceDataProducer.Recorder
  ) {
