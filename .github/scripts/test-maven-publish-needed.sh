...
EOF
  ...
  # A second publishing module, in another directory.
...
# The `--train` assertions were here: `data/*` versioned on a line of its own and the guard
# answered per line. Those modules publish from compose-preview-daemon since #5336.

echo "== a tree with no publishing modules fails open"
EMPTY="${tmp}/empty"
make_repo "${EMPTY}"
# Every publishing module — the case under test is an enumeration that finds nothing at all.
...
count="$(printf '%s\n' "${paths}" | wc -l | tr -d ' ')"
if [ "${count}" -gt 20 ]; then ok "--print-paths lists ${count} paths"; else bad "--print-paths listed only ${count}"; fi
