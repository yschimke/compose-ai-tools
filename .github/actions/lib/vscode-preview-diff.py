...
The two `preview-harness` suites — the VS Code panel's under
`vscode-extension/`, the serve pages' under `preview-server/` — each
write `<fixture>.<theme>.png` files into their own `out/`; the calling
action merges both into one directory before invoking this script. Each
...
# and `preview-server/preview-harness/pages-snapshot.spec.mjs`.
