#!/usr/bin/env python3
"""Generate serve-preview baselines or compare against them.

`preview-server/preview-harness` writes `<fixture>.<theme>.png` into its
`out/`, and each (fixture, theme) pair is the unit of comparison. A much
simpler shape than the gradle-preview pipeline (no per-preview JSON
manifest, no nested module dirs), which is why this stays a separate
helper rather than a mode of `compare-previews.py`.

It used to serve two producers — this one and the VS Code panel's
harness, merged into one directory by the calling action. The extension
moved to yschimke/compose-preview-vscode and took its harness with it,
so there is one producer here now. That repository runs a copy of this
script against its own captures; a real bug fixed here is worth carrying
across.
...
# `<fixture>.<theme>.png` — see `preview-server/preview-harness/pages-snapshot.spec.mjs`.
...
def cmd_generate(args: argparse.Namespace) -> int:
    ...
    for name, meta in entries.items():
        ...
        # to `serve-preview/main` (Chromium AA jitter, see
    ...
    lines = [
        "# Serve page-capture baselines",
        "",
        "Auto-generated by `.github/workflows/serve-preview-baselines.yml`.",
...
def cmd_compare(args: argparse.Namespace) -> int:
    ...
    if not new and not changed and not removed:
        print("<!-- serve-preview-diff -->")
        print("## Serve page-capture diff")
        print("")
    ...
    out: list[str] = []
    out.append("<!-- serve-preview-diff -->")
    out.append("## Serve page-capture diff")
    out.append("")
    summary = []
    if changed:
...
def main(argv: list[str]) -> int:
    ...
    # extracted from `serve-preview/main` via `git archive`). When
