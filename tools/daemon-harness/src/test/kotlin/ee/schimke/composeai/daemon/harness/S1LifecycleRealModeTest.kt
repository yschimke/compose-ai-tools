 ...
 * `:renderer-desktop-daemon` `DaemonMain` via [RealDesktopHarnessLauncher] rather than
 * [FakeDaemonMain].
 *
 * **Skipped under fake mode.** `JUnit Assume.assumeTrue(harnessHost == "real")` short-circuits the
 * test under the default `-Pharness.host=fake`. Run with `./gradlew :tools:daemon-harness:test
 * -Pharness.host=real --tests "*S1LifecycleRealModeTest"`.
 *
 * **Auto-capture-on-first-run baseline.** A baseline PNG lives at
 * `tools/daemon-harness/baselines/desktop/s1/red-square.png`. On the first run the test captures
...
class S1LifecycleRealModeTest {
  ...
  fun s1_lifecycle_real_mode_happy_path() {
    ...
    // to the `RedSquare` composable in `:renderer-desktop-daemon`'s testFixtures source set
    ...
    // see `tools/daemon-harness/build.gradle.kts`.
