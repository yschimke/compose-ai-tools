...
| Render mode | a11y (daemon-only — opt in via the VS Code chip toggle or `compose-preview a11y`; standalone `renderPreviews` Gradle task does not produce a11y data) |
| Cost | low |
...
The standalone `:renderAllPreviews` Gradle task does NOT produce a11y
