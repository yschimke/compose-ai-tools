...
/**
 * A resolved theme: one generated provider. `name` and `group` become the catalog annotation's
 ...
 *   imports: string[], wear: boolean}>, errors: string[]}}
 */
export function resolveThemes(spec) {
  ...
  declared.forEach((entry, index) => {
    ...
    if (entry?.wear !== undefined && typeof entry.wear !== "boolean") {
      errors.push(`${where}.wear: expected a boolean`);
      return;
    }
...
export function renderKotlin(themes, { packageName = GENERATED_PACKAGE } = {}) {
  ...
  const header = [
    ...
    "//",
    "// An imported project cannot declare theme-catalog providers itself — it has no dependency on",
  ...
  const bodies = themes.map((t) => {
    ...
    const annotation = t.wear ? "WearThemeCatalog" : "ThemeCatalog";
    return [
      `@${annotation}(${args.join(", ")})`,
