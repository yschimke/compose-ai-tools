...
// The builder catalog, generated by discovery from the record, the cover sheet and the catalog's
...
if (uiBuilderCatalog) {
  console.log(
    `[${spec.system}] published builder catalog → ${uiBuilderCatalog.path} ` +
      `(${uiBuilderCatalog.catalogId}, platform ${uiBuilderCatalog.platform}, ` +
  ...
  if (uiBuilderCatalog.diagnostics > 0) {
    ...
    console.log(
      `[${spec.system}] builder catalog carries ${uiBuilderCatalog.diagnostics} diagnostic(s) — ` +
        `see \`diagnostics\` in ${uiBuilderCatalog.path}`,
    );
  }
}

{
  ...
  // Declared the same way, and absent for the same reason: a consumer that finds no
