 ...
 *
 * `"capture": "animated"` is the declaration: the entry stays in the spec (so the gap is recorded
 * where the inventory lives), it is excluded from the candidate join like any other PNG-less
 * preview, but it does NOT count as a missing render. `"static"` (the default) keeps the strict
 * behaviour — a spec entry that renders nothing still sinks the publish, which is the point of the
 * gate.
 *
...
export const CAPTURE_MODES = ["static", "animated"];
...
/** Whether an entry declares itself non-static, i.e. exempt from the missing-render gate. */
export function isAnimatedCapture(entry) {
  return captureMode(entry) === "animated";
}
