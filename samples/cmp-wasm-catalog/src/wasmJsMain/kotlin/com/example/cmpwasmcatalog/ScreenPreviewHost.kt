...
/**
 * Where a [Screen] is composed for the builder's preview pane.
 *
 ...
 *
 * ### Why this is an interface and not a call to `CatalogScreen`
 *
 ...
 * `Screen`, and where it gets composed is a deployment question, not a modelling one. Hard-wiring
 * `CatalogScreen` would have made the browser-only path structural, and unpicking it later means
 * touching every part of the builder rather than one binding.
 *
...
internal interface ScreenPreviewHost {
  /** Compose (or display) [screen] in the builder's preview pane. */
...
internal object WasmCatalogPreviewHost : ScreenPreviewHost {
  ...
  @Composable
  override fun Preview(screen: Screen, modifier: Modifier) {
    CatalogScreen(screen, modifier)
  }
}
