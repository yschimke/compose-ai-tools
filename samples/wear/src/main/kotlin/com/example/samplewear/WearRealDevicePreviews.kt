 ...
 *   well documented, the reported density is not, and guessing it is the mistake this annotation is
 *   fixing.
 *
 * Note what the table implies about the catalog: `id:wearos_xl_round` is **240 dp**, which is 480
 * px at 320 dpi — a combination no device above ships. Studio's XL round is a synthetic ceiling,
 * not a watch. `@WearPreviewDevices` (192 / 227 dp) is likewise catalog geometry. Use those to
 * check you survive the extremes; use this annotation to check the sizes real wrists report.
 * [WearDeviceMatrixPreview] keeps the catalog ids covered.
 *
 * The Samsung rows render 2 px narrower than the physical panel (211 dp × 2.125 = 448 px, not 450).
 * That is inherent: `screenWidthDp` is an integer, so the device itself truncates 211.76 dp to 211,
 * and those last pixels are not addressable in dp. The **dp column is the contract** — it's the
 * space a layout is handed — and the pixel count follows from it.
 *
